...
(function () {
  ...
  var HINTS         = ["onKey の ENTER は if (isValidMove(cursorRow, cursorCol, currentPlayer)) の中で placeAt を呼ぶだけ。", "flip は方向ごとに 2 段構え。Phase 1: while (board[r][c] == opponent) r += dr; c += dc; で相手の端を探す。", "Phase 2: ループ脱出後、そこが player の石なら、逆向きに 1 マスずつ戻しながら board[r][c] = player。(row,col) に戻ったら止める。"];
  var TESTS         = [{"name": "↑ + Enter で (col=3, row=2) に黒石が置かれる", "type": "press-sequence-then-cell-color", "keys": [38, 13], "intervalMs": 100, "waitMs": 200, "col": 3, "row": 2, "color": [0, 0, 0], "tolerance": 20}, {"name": "挟まれた (col=3, row=3) が白から黒に反転している", "type": "cell-color", "col": 3, "row": 3, "color": [0, 0, 0], "tolerance": 20}, {"name": "置いた直後の黒の手番 (まだ交代しないので) で (5,4) にヒントが残る", "type": "cell-color", "col": 5, "row": 4, "color": [255, 200, 50], "tolerance": 30}];
  var STEP_ID       = "step-05";
