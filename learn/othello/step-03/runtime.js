...
(function () {
  ...
  var HINTS         = ["onKey の典型形: if (key == Key.LEFT && cursorCol > 0) cursorCol--;  — 4 方向分並べる。", "太い枠は strokeRect を 2-3 本重ねて作る。例: (cx, cy, CELL, CELL) と (cx+1, cy+1, CELL-2, CELL-2)。", "境界チェックを忘れると cursorCol が -1 や 8 になって next step で落ちる。端で止めるのが正解。"];
  var TESTS         = [{"name": "初期状態でカーソル枠が画面に出ている (HIGHLIGHT色)", "type": "area-has-color", "x1": 148, "y1": 148, "x2": 202, "y2": 202, "color": [255, 220, 90], "tolerance": 30}, {"name": "↓ キー 1 回でカーソルが下に移動する", "type": "press-keys-then-area-has-color", "keyCode": 40, "count": 1, "x1": 148, "y1": 198, "x2": 202, "y2": 252, "color": [255, 220, 90], "tolerance": 30}, {"name": "→ キー 3 回でカーソルが右に移動する", "type": "press-keys-then-area-has-color", "keyCode": 39, "count": 3, "x1": 298, "y1": 148, "x2": 352, "y2": 202, "color": [255, 220, 90], "tolerance": 30}];
  var STEP_ID       = "step-03";
