...
(function () {
  ...
  var HINTS         = ["base case 2 つ: depth == 0 → return evaluate(AI); 合法手が無く相手もパスなら終局 → 同じく return evaluate(AI);", "maximizing = (player == AI); 最大化か最小化かは player で決まる。int best = maximizing ? Integer.MIN_VALUE : Integer.MAX_VALUE;", "再帰: int score = minimax(depth - 1, opponent); Math.max または Math.min で best を更新。snapshot/restore を忘れない。"];
  var TESTS         = [{"name": "↑ + Enter で黒が (col=3, row=2) に着手する", "type": "press-sequence-then-cell-color", "keys": [38, 13], "intervalMs": 100, "waitMs": 300, "col": 3, "row": 2, "color": [0, 0, 0], "tolerance": 20}, {"name": "minimax AI が白の応手を 2 秒以内に選ぶ (合法手のいずれかに白石)", "type": "press-sequence-then-any-cell-color", "keys": [], "intervalMs": 100, "waitMs": 2000, "cells": [[2, 2], [2, 4], [4, 2]], "color": [255, 255, 255], "tolerance": 20}];
  var STEP_ID       = "step-09";
